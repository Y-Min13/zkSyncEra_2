...
def create_xml(log_file):
    if os.path.exists(log_file) is False:
        ...
        worksheet = workbook.create_sheet('NFT transactions')
        worksheet.cell(row=1, column=1).value = "№ кошелька"
...
class LogNFT(object):
    def __init__(self, wallet, txn_hash, nft, value, param, balance_st, balance_end, script_time):
        self.wallet_num = wallet.wallet_num
        self.address = wallet.address
        self.operation = wallet.txn_num
        self.txn_hash = txn_hash
        self.nft = nft.nft_name
        self.value = value
    ...
    def write_log(self):
        while True:
            try:
                ...
                worksheet = workbook['NFT transactions']
                ...
                worksheet.cell(row=last_row + 1, column=4).value = self.nft
                ...
                worksheet.cell(row=last_row + 1, column=8).value = self.value
                worksheet.cell(row=last_row + 1, column=8).number_format = '0.00000'

                worksheet.cell(row=last_row + 1, column=9).value = self.param
                worksheet.cell(row=last_row + 1, column=10).value = self.script_time
